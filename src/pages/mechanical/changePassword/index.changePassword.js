...
function ChangePasswordMechanical(){
    const navigation = useNavigation();

    const {person} = useContext(PersonContext);
    const {changePasswordMechanical} = useContext(MechanicalContext);
    ...
    async function handleChangePasswordMechanical(){
        if(newPassword != '' && newPassword != null && confirmedPassword != '' && confirmedPassword != null && oldPassword != null && oldPassword != ''){
            ...
            if(newPassword == confirmedPassword){
                ...
                setInvalidPassword(false);
                setErrorSamePassword(false);
                const response = await changePasswordMechanical(oldPassword, confirmedPassword, person.mechanicalToken);
                if(response == false){
                    ...
                    setInvalidPassword(false);
                    setErrorLabel(null);
                    navigation.navigate('OptionsMechanicalProfile');
                }
            }else{
                setInvalidPassword(false);
                setErrorFields(false);
    ...
    return(
        <View style={{flex: 1, backgroundColor: '#fff'}}>
            <View style={{alignSelf: 'center', width: 370, marginTop: 32}}>
                ...
                autoCapitalize="none"
                />
                <TouchableOpacity onPress={() => {handleChangePasswordMechanical()}} 
...
export default ChangePasswordMechanical;